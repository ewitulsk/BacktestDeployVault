...
- All privileged contract roles must support transfer/rotation to multisigs after deployment. Stateful first-party EVM contracts must use standard, validated upgradeable proxies; Solana programs must retain transferable native upgrade authority. Follow the contracts spec for initialization, storage compatibility, governance, and handoff tests.
